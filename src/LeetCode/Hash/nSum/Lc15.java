    ...
    public static List<List<Integer>> nSumTarget(int[] nums, int n,int start,int target){
        ...
        if(n!=2){
            ...
            for(int i= start; i < numsLen; i++){
                ...
                List<List<Integer>> subSum = nSumTarget(nums,n-1, i+1,target-nums[i]);

                //如果求解(n-1）Sum问题所得结果是无效的，说明不存符合条件的结果，直接往上一层返回null
